...
calculate_contiguity_score <- function(nc_data, neighbours_queen) {
  ...
  # Perfect: total_components = n_districts (1.0 = 100% improvement)
  # Worst case: total_components = 2 * nrow(nc_data) (0.0 = 0% improvement, can go negative)
  worst_case <- 2 * nrow(nc_data)
  perfect_case <- n_districts
  
  if (worst_case == perfect_case) {
    contiguity_score <- 1.0
  } else {
    contiguity_score <- (worst_case - total_components) / (worst_case - perfect_case)
  }
...
# Contiguity progress plot
contiguity_history <- numeric(length(score_history))
cat("📊 Calculating contiguity history (this may take a moment)...\n")

# Calculate contiguity for initial state
initial_contiguity <- calculate_contiguity_score(nc, neighbours_queen)
contiguity_history[1] <- initial_contiguity$score

# For demonstration, we'll just show the final contiguity change
# (calculating full history would require storing all configurations)
cat("Contiguity Analysis:\n")
cat(sprintf("  Initial: %.4f (%d components)\n", initial_contiguity$score, initial_contiguity$total_components))
cat(sprintf("  Final: %.4f (%d components)\n", final_metrics["contiguity"], final_components))
cat(sprintf("  Change: %.4f (%.1f%% improvement in contiguity score)\n", 
            final_metrics["contiguity"] - initial_contiguity$score,
            ((final_metrics["contiguity"] - initial_contiguity$score) / initial_contiguity$score) * 100))

cat("\n✅ Enhanced optimization with contiguity breaking complete!\n")
cat("Key innovations:\n")
cat("  • Removed integral precinct filtering - all border precincts eligible\n")
cat("  • Multiple swaps per edit (1 to", s, ") enabling contiguity breaking\n")
cat("  • Contiguity score integrated as 5th optimization target\n")
cat("  • Enhanced simulated annealing with detailed temperature tracking\n")
cat("  • Comprehensive progress reporting including contiguity metrics\n")